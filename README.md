...
Vite + React + TypeScript 的 PSTI 早期产品。默认使用浏览器 mock 数据层；配置后可切换到 Supabase。
...
- `share_events`：`result_opened` 和 `share_clicked` 两类结果页事件。

`state_summary` 保存本次选择造成的字段变化、前后值和摘要，不保存姓名、手机号、邮箱等个人信息。

## 数据后端

没有环境变量时自动使用 `localStorage` mock adapter，数据位于：
...
2. 执行 `supabase/migrations/20260612000000_create_test_tracking.sql`。
...
Supabase adapter 使用匿名 Auth 和 RLS 限制测试写入。公开结果通过 `get_public_test_result` RPC 按 UUID 返回报告字段，不开放结果表的公开遍历。
...
4. 刷新结果页，确认报告仍可读取，并检查 `shareEvents` 新增 `result_opened`。
5. 点击“分享结果”，确认新增 `share_clicked`。
